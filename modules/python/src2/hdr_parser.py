...
class CppHeaderParser(object):
    ...
    def __init__(self, generate_umat_decls=False, generate_gpumat_decls=False, generate_aclmat_decls=False):
        self._generate_umat_decls = generate_umat_decls
        self._generate_gpumat_decls = generate_gpumat_decls
        self._generate_aclmat_decls = generate_aclmat_decls
                ...
                if stack_top[self.PROCESS_FLAG]:
                    ...
                    if decl:
                        if stmt_type.startswith("enum"):
                            ...
                            if self._generate_aclmat_decls and "cv.cann" in decl[0]:
                                # If function takes as one of arguments Mat or vector<Mat> - we want to create the
                                # same declaration working with AclMat
                                args = decl[3]
                                has_mat = len(list(filter(lambda x: x[0] in {"Mat", "vector_Mat"}, args))) > 0
                                if has_mat:
                                    _, _, _, aclmat_decl = self.parse_stmt(stmt, token, mat="cann::AclMat", docstring=docstring)
                                    if aclmat_decl != decl:
                                        decls.append(aclmat_decl)
...
if __name__ == '__main__':
    parser = CppHeaderParser(generate_umat_decls=True, generate_gpumat_decls=True, generate_aclmat_decls=True)
    decls = []
